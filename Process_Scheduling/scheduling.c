...
int first_come_first_served(process proc[], int t)
{
  ...
  int next = 1000;
  int ret = -1;
  /*Run through all the current proccesses sequentially*/
  for(i =0;i<NUM_PROCESSES; i++){ 
	/*Find the process with the smallest arrival time that is smaller than the current time and not finished*/
    if(proc[i].arrivaltime < next && proc[i].arrivaltime <= t && !proc[i].finished){
      next = proc[i].arrivaltime;
      ret = i;
...
int shortest_remaining_time(process proc[], int t)
{
  ...
  int next =10000;
  int ret = -1;
  /*Run through all the current processes sequentially*/
  for(i =0; i < NUM_PROCESSES; i ++){
	/*Find the process with the shortest remaning time*/
    if(proc[i].remainingtime < next && proc[i].arrivaltime <= t && !proc[i].finished){
      next = proc[i].remainingtime;
      ret = i;
    }
...
int round_robin(process proc[], int t)
{
 ...
 /*Run through all the current processes sequentially*/
 for(i = 0; i < NUM_PROCESSES; i++){
   j++; //J is the counter for the previous node
   /*If we reached the end of the list then we start over*/
   if(j > NUM_PROCESSES-1 ){
     j =0;
   }
   /*If we reached a process that is not finised and has a shorter arival time then what we are on return it and save it*/
   if(!proc[j].finished && proc[j].arrivaltime <= t){
      prevNode = j;
      return prevNode;
    } 
...
int round_robin_priority(process proc[], int t)
{
  ...
  /*Finds the current highest priority*/
  for(k =0; k < NUM_PROCESSES; k++){
    if(!proc[k].finished && highestPrio < proc[k].priority && proc[k].arrivaltime <= t ){
      highestPrio = proc[k].priority;
    }
  }
// printf("HP: %d\n", highestPrio);
 /*Run through all the current processes sequentially, BELOW: Same as "round_robin"*/
 for(i =0; i < NUM_PROCESSES; i++){
    j++;
    if(j > NUM_PROCESSES -1){
       j =0;
    }
    if(!proc[j].finished && proc[j].arrivaltime <= t &&  proc[j].priority == highestPrio) {
      prevNode2 = j;
      return prevNode2;
    }
